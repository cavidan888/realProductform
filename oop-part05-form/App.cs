...
namespace oop_part05_form
{
    public partial class App : Form
    {
        ...
        private void showButton_Click(object sender, EventArgs e)
        {
            ...
            {
                productListView.Text += item.Id + " . " + item.Name + " / "+ item.CategoryName + " / " +item.Price + dateTimePicker1.Value.ToShortDateString() +  "\r\n";
            }
        }
        ...
        private void addCategoryButton_Click(object sender, EventArgs e)
        {
            ...
            {
                categoryList.Items.Add(item.Name);
                prdctsBox.Items.Add(item.Name);
            }
        }

        private void prdctsbtn_Click(object sender, EventArgs e)
        {


            foreach (var item in Entity.Products)
            {
                if (item.CategoryName == prdctsBox.SelectedItem.ToString())
                {
                    productListView.Text += item.Name;
                }
            }
        }

        private void prdctsBox_SelectedIndexChanged(object sender, EventArgs e)
        {
